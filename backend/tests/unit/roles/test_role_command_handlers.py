...
class TestCreateRole:
    ...
    def test_create_role_with_existing_name(self, messagebus, roles_default_role):
        messagebus.uow.role_repository.add(roles_default_role)
        with pytest.raises(RoleAlreadyExists):
            messagebus.handle(CreateRole(roles_default_role.name))


class TestAddPermissionToRole:
    def test_add_permission_to_role(self, messagebus, roles_default_role, permission):
        messagebus.uow.role_repository.add(roles_default_role)
        messagebus.handle(
            AddPermissionToRole(
                role_name=roles_default_role.name, permission_name=permission.name
            )
        )
        assert permission in roles_default_role.permissions
    ...
    def test_add_permission_to_role_with_non_existing_permission(
        self, messagebus, roles_default_role
    ):
        messagebus.uow.role_repository.add(roles_default_role)
        messagebus.handle(
            AddPermissionToRole(
                role_name=roles_default_role.name,
                permission_name="non_existing_permission",
            )
        )
        assert Permission("non_existing_permission") in roles_default_role.permissions
